...
public class GameFieldPanel extends JPanel 
{
				...
				squarePanel.addMouseListener(new MouseAdapter() {
					...
					public void mousePressed(MouseEvent e) {
						...
						if (!swapPressed) 
						{
							...
							if (match.getSelectedTile() != null	&& !sq.getOccupied()) 
							{
							...
							else if (match.getSelectedTile() == null && sq.getOccupied()
									&& match.getSquare(sq.getXValue(), 
											sq.getYValue()).getTile().getJustPlayed()) 
							{
								match.moveTileFromBoardToHand(sq.getXValue(), sq.getYValue());
								sq.removeImage();
							}
						}
					}});
	...
	public void suffleHand() 
	{
		for (int i = 0; i < 10; i++) 
		{
			TilePanel tp = tiles.get((int) (0 + (Math.random())	* ((tiles.size() - 0))));
			tiles.remove(tp);
			tiles.add(tp);
		}
