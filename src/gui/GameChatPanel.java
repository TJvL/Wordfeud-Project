...
public class GameChatPanel extends JPanel implements ActionListener 
{
	...
	{				// I get the player object references and the gameID of this match from the class that invokes this constructor (Wouter, Mike)
		this.user1 = "You";
	...
	public synchronized void checkForMessages() 
	{
		...
		for (String a : chatMessages) 
		{
			...
			{					//if it IS the first incoming message since startup of application				
			...
			{																//if it's NOT the first incoming message since startup of application
								//System.out.println("this is NOT the first message");		TEST CODE
				if (!latestUpdatedMessageTimeDate.equals(dateTime) && !latestUpdatedMessage.equals(message)) 
				{ 		//if the incoming message is NOT already in the chatscreen
