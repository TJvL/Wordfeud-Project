	...
	public void run() {
		...
		while (running) {
			if (match != null) {
				...
				final int enemyScore = DatabaseHandler.getInstance().score(
						gameID, enemyName);
				final int ownScore = DatabaseHandler.getInstance().score(
						gameID, ownName);
				...
				final String gameStatus = DatabaseHandler.getInstance()
						.getGameStatusValue(gameID);
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						try {
							// If the game status is "Playing" then the normal
							// turn
							// check is done
							if (!gameStatus
								...
								if (isMyTurn) {
									// Check if the turn has just been swapped
									// and we
									...
									// Check if the current macth has enough
									// letters in
									// the jar so users may swap their letters.
									if (!isSwapAllowed) {
										buttonPanel.disableSwap();

									}
									isTurnJustSwapped = false;
									// else update the GUI in a way so the
									// current user
									// can't use buttons he shouldn't be using
									// because
								...
								// else check why the game is not "Playing"
								// anymore.
							} else {
								// if the game is not "Playing" anymore then
								// check if
								// it's "Finished".
								if (gameStatus
										.equals(WordFeudConstants.GAME_STATUS_FINISHED)) {
									// if the current users score is lower then
									// his
									// opponents score, tell the GUI to tell the
									// user he
									// lost.
									if (enemyScore > ownScore) {
										...
										// else tell the GUI to tell the user he
										// has
										// won.
									} else {
										JOptionPane
								...
								if (gameStatus
										.equals(WordFeudConstants.GAME_STATUS_RESIGNED)) {
									// if the game is resigned because the
									// current user
									// resigned it, tell he GUI to tell the user
									// he Lost
									// because he resigned.
									if (didISurrender) {
										JOptionPane
										...
										// else tell the user his opponent
										// resigned and
										// he won.
									} else {
										JOptionPane
								...
								// Finnaly tell the GUI that the user cannot
								// play words
								// anymore, like it wasn't his turn.
								buttonPanel.setTurn(false);
								buttonPanel.disableSurrender();
								// Break the loop for it is no longer necessary
								// to
								// update the GUI with the newest data.
								running = false;
							}
							scorePanel.repaint();
